...
<body class="bg-gradient-to-br from-green-50 to-blue-50 min-h-screen p-4">
    ...
    <div id="mainScreen" class="max-w-4xl mx-auto space-y-6 hidden">
        ...
        <!-- ルート選択 -->
        <div id="routeSelection" class="bg-white rounded-2xl shadow-lg p-6 hidden">
            <h2 class="text-lg font-bold mb-4">🗺️ 他のルート候補</h2>
            <div id="routeButtons" class="space-y-3">
                <!-- ルートボタンがここに追加される -->
            </div>
        </div>
    ...
    <script>
        ...
        async function createRoute() {
            ...
            try {
                ...
                if (data.status === 'OK' && data.routes && data.routes.length > 0) {
                    ...
                    console.log(`${allRoutes.length}個のルートが見つかりました`);
        ...
        function showRouteSelection() {
            ...
            allRoutes.forEach((route, index) => {
                ...
                button.className = 'w-full p-4 rounded-xl border-2 border-gray-200 hover:border-green-500 hover:bg-green-50 transition-all text-left';
                button.innerHTML = `
                    <div class="flex justify-between items-center">
                        <div>
                            <div class="font-bold text-gray-800">ルート ${index + 1}</div>
                            <div class="text-sm text-gray-600 mt-1">${distanceKm}km · ${actualSteps.toLocaleString()}歩 · ${timeMinutes}分</div>
                        </div>
                        <div class="text-2xl">${index === 0 ? '🟢' : index === 1 ? '🔵' : '🟠'}</div>
                    </div>
                `;
                button.onclick = () => {
                    displayRoute(index);
            ...
            // 最初のボタンをハイライト
            const firstButton = routeButtonsContainer.querySelector('button');
            if (firstButton) {
